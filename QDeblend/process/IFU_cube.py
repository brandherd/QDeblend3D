...
class IFUcube(object):
    ...
    def deblendQSOHost(self, center,qso_mask, broad_region, cont_region, iter, mode,  eelr_mask=None,
                       interpolate_cont=False, subtract_region=None,  radius=None, factor=None,
                       host_image=None, showProgress=None):
        ...
        if radius is not None and radius!=0.0:
            ...
            radius_mask = numpy.fromfunction(lambda i, j: numpy.sqrt((i-center[0])**2+(j-center[1])**2)<=radius,
                                             (self.yDim,  self.xDim), dtype=int)
        else:
        ...
        for i in range(iter):
            ...
            if not abort:
                ...
                if not interpolate_cont:
                    ...
                    points_x = [numpy.median(self.wave[wave_select_cont1]),
                                numpy.median(self.wave[wave_select_cont2]),
                                numpy.median(self.wave[wave_select_cont2])]
                    point_x1 = points_x[0]
                    point_x2 = points_x[1]      
                ...
                if not interpolate_cont:
                    ...
                    lin_cube = self.wave[:, numpy.newaxis, numpy.newaxis] * a1[numpy.newaxis, :, :] + \
                               b1[numpy.newaxis, :, :]
