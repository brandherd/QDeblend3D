...
class mask(object):
    ...
    def __init__(self,dim,init_mask_array=None):
        ...
        self.dim = dim
        if init_mask_array!=None and self.dim == init_mask_array.shape:
    ...
    def maskBox(self, center, boxsize):
        if (boxsize-1)%2 == 0 and center[0]-(boxsize-1)//2>=0 and center[0]+(boxsize-1)//2+1<=self.dim[0] and center[1]-(boxsize-1)//2>=0 and center[1]+(boxsize-1)//2+1<=self.dim[1]:
            self.mask[center[0]-(boxsize-1)//2:center[0]+(boxsize-1)//2+1, center[1]-(boxsize-1)//2:center[1]+(boxsize-1)//2+1] = True
    
    def unmaskBox(self, center, boxsize):
        if (boxsize-1)%2 == 0 and center[0]-(boxsize-1)//2>=0 and center[0]+(boxsize-1)//2+1<=self.dim[0] and center[1]-(boxsize-1)//2>=0 and center[1]+(boxsize-1)//2+1<=self.dim[1]:
            self.mask[center[0]-(boxsize-1)//2:center[0]+(boxsize-1)//2+1, center[1]-(boxsize-1)//2:center[1]+(boxsize-1)//2+1] = False
...
class displayImgMask(mask):
    ...
    def __init__(self, canvas,  dim, init_mask_array=None, visible=False, hatch=None, fill=True, lw=1.0, color='0.7',zorder=3, alpha=0.5):
        if len(dim)==2:
            ...
            self.zorder = zorder
            self.canvas = canvas
            if init_mask_array!=None:
                for i in range(dim[0]):
                    for j in range(dim[1]):
                        if self.mask.mask[i, j]==True:
                            self.area.append(matplotlib.patches.Rectangle(xy=(j-0.5, i-0.5), width=1.0, height=1.0, color=color, lw=lw, ec=color, fc=color,fill=fill, visible=visible, hatch=hatch, zorder=zorder, alpha=alpha))
    ...
    def maskArray(self, array):
        ...
        for i in range(dim[0]):
            for j in range(dim[1]):
                if self.mask.mask[i, j]==True:
                    self.area.append(matplotlib.patches.Rectangle(xy=(j-0.5, i-0.5), width=1.0, height=1.0, color=self.color, lw=self.lw, ec=self.color, fc=self.color,fill=self.fill, visible=self.visible, hatch=self.hatch, zorder=self.zorder, alpha=self.alpha))
                    self.map[(i, j)]=self.masked
                    self.canvas.axes.add_artist(self.area[self.masked-1])
                    self.masked+=1
        self.canvas.fig.canvas.draw()
    def maskPixel(self, position):
            ...
            self.area.append(matplotlib.patches.Rectangle(xy=(position[1]-0.5, position[0]-0.5), width=1.0, height=1.0, color=self.color, ec=self.color, fc=self.color,fill=self.fill, hatch=self.hatch, visible=self.visible, zorder=self.zorder, alpha=self.alpha))
            self.canvas.axes.add_artist(self.area[self.masked-1])
            self.masked+=1
            self.canvas.fig.canvas.draw()
    ...
    def setMask(self, mask):
        ...
        for i in range(self.dim[0]):
            for j in range(self.dim[1]):
                if self.mask.mask[i, j]==True:
                    self.area.append(matplotlib.patches.Rectangle(xy=(j-0.5, i-0.5), width=1.0, height=1.0, color=self.color, ec=self.color, fc=self.color,fill=self.fill, visible=self.visible, hatch=self.hatch, zorder=self.zorder, alpha=self.alpha))
                    self.map[(i, j)]=self.masked
                    self.canvas.axes.add_artist(self.area[self.masked-1])
                    self.masked+=1
        self.canvas.fig.canvas.draw()
...
class displaySpecRegion(object):
    def __init__(self, canvas, waveLimit=None,  visible=False, hatch=None, fill=True, linestyle='-',  color='0.7',zorder=3,lw=1.0, alpha=0.5, picker=5):
            ...
            if waveLimit!=None:
                self.area=self.canvas.axes.axvspan(self.waveLimit[0], self.waveLimit[1], color=color, linestyle=linestyle, lw=lw, ec=color, fc=color,fill=fill, visible=visible, hatch=hatch, zorder=zorder, alpha=alpha, picker=picker)
    ...
    def setLimit(self, waveLimit):
        if waveLimit!=None:
            ...
            if self.area == None:
                self.area=self.canvas.axes.axvspan(self.waveLimit[0], self.waveLimit[1], color=self.color, ec=self.color, fc=self.color,fill=self.fill, visible=self.visible, hatch=self.hatch, zorder=self.zorder, alpha=self.alpha)
                ...
                vertex = [[self.waveLimit[0], 0.0], [self.waveLimit[0], 1.0], [self.waveLimit[1], 1.0], [self.waveLimit[1], 0.0], [self.waveLimit[0], 0.0]]
    ...
    def isEmpty(self):
        if self.waveLimit == None or self.waveLimit[1]==self.waveLimit[0]:
            return True
        else:
            return False
